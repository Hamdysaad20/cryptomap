...
function NavNavbarMobol(props) {
  ...
  const navname = ["home", "video", "shop", "group"];
  ...
  const navtext = [navsvg1, navsvg2, navsvg3, navsvg4];
  ...
  const arrlistnav = [svg1, svg2, svg3, svg4, svg5, svg6];
      ...
      <header className=' z-50 block sm:hidden mt-[0px] overflow-hidden lg:max-w-9xl sticky top-0   bg-white shadow-sm pt-2'>
        <div className=' mx-3 gap-4  md:mx-15 xl:mx-16 grid md:outline-hidden grid-cols-5    text-center'>
          ...
          <div onClick={toggle} className={showMe ? x : ""}>
            <div className=' cursor-pointer hover:bg-red-100  noselect py-2 ease-out  transition  duration-200  rounded-lg  '>
              <svg
                className='h-6 pt-1 w-6 mx-auto'
                xmlns='http://www.w3.org/2000/svg'
                viewBox='0 0 24 24'
          ...
          <div className='mx-2 '>
            {Navlist.students.map((item, i) => (
              <div
                ...
                <div className='sm:hidden  right-0 '>{arrlistnav[i]}</div>
              </div>
            ))}
